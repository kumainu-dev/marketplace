...
export const getStarlPrice = async () => {
    const clinet = new CoinGecko();

    try {
        const response = await clinet.simple.price({ ids: 'starlink', vs_currencies: 'USD' });
        if (response && response.success) return parseFloat(response.data['starlink'].usd); 
    } catch(err) {
        console.error(err);
    }
    return 0;
